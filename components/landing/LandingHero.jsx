  ...
  return (
    <section className="min-h-screen pt-[72px] flex flex-col">
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-2 border-b border-black">
        ...
        {/* ── Left: copy ── */}
        <div className="flex flex-col justify-between p-10 lg:p-16 border-r border-black">
          <div>
            {/* System badge */}
            ...
            <h1 className="font-black leading-[.95] tracking-tight mb-8"
                style={{ fontSize: 'clamp(52px, 7vw, 73px)' }}>
              PRECISION<br/>INTELLIGENCE.
            </h1>

            <p className="text-base leading-relaxed opacity-60 max-w-md mb-10">
            ...
            <div className="flex items-center gap-6">
              <a href="/auth"
                 className="inline-flex items-center gap-3 px-8 py-4 bg-black text-white
                            text-[11px] font-bold tracking-widest uppercase border border-black
                            hover:bg-white hover:text-black transition-all group">
                Enter System
                <ArrowRight size={14}
          ...
          {/* Stats */}
          <div className="grid grid-cols-3 border-t border-black pt-8 mt-16">
            {STATS.map(s => (
              <div key={s.label}>
                <div 
        ...
        {/* ── Right: AI Figure ── */}
        <div className="relative flex items-center justify-center bg-gray-50 p-12 overflow-hidden">
          ...
          {['UNIT-01','SYS-CORE','STATUS: ACTIVE','AETHER/AI'].map((l, i) => (
            <span key={l}
              className={`absolute text-[9px] font-bold tracking-[.15em] uppercase opacity-25
                ${i===0?'top-5 left-5':i===1?'top-5 right-5':i===2?'bottom-5 left-5':'bottom-5 right-5'}`}>
              {l}
            </span>
          ))}
          <AIFigure size={340} />
